...
using airfix::windows::AirfixWindowsTexturePackStatus;
  ...
  TemporaryDirectory() {
    ...
    for (std::uint64_t attempt = 0U; attempt < 100U; ++attempt) {
      path_ = std::filesystem::temp_directory_path() /
              ("airfix-texture-pack-session-tests-" + std::to_string(stamp) +
...
void testValidSessionAndFixedFailures() {
  ...
  const auto missingManifest =
      airfix::windows::openAirfixWindowsTexturePack(root, "missing.jsonl");
  require(!missingManifest.success() &&
  ...
  const auto invalidManifest =
      airfix::windows::openAirfixWindowsTexturePack(root, "invalid.jsonl");
  require(!invalidManifest.success() &&
              invalidManifest.status ==
                  AirfixWindowsTexturePackStatus::manifestInvalid,
          "malformed manifest did not fail with a fixed category");

  const auto unsafeManifest =
      airfix::windows::openAirfixWindowsTexturePack(root, "../reviewed.jsonl");
  require(!unsafeManifest.success() &&
              unsafeManifest.status ==
                  AirfixWindowsTexturePackStatus::invalidConfiguration,
          "escaping manifest path did not fail closed");

  const auto relativeRoot = airfix::windows::openAirfixWindowsTexturePack(
      "relative-root", "reviewed.jsonl");
  require(!relativeRoot.success() &&
              relativeRoot.status ==
                  AirfixWindowsTexturePackStatus::invalidConfiguration,
...
int main() {
  try {
    ...
    std::cout << "Airfix Windows texture pack tests passed\n";
    return 0;
