...
struct RenderPresentationSettings final {
  float renderScalePercent{native_render_policy::defaultRenderScalePercent};
  ...
  // This is the durable user request. The texture package resolver owns the
  // separate effective mode for an active mission snapshot.
  texture::TextureMode textureMode{texture::TextureMode::classic};
  ...
  operator==(const RenderPresentationSettings &,
...
struct RenderPresentationSettingsOverride final {
  ...
  std::optional<texture::TextureMode> textureMode;
...
struct RenderPresentationSettingsDelta final {
  ...
  bool textureModeChanged{};
  bool uiLayoutChanged{};

  [[nodiscard]] constexpr bool anyChanged() const noexcept {
    return scaleTargetsChanged || layoutChanged || diagnosticsChanged ||
           visualProfileChanged || textureModeChanged || uiLayoutChanged;
  }

  [[nodiscard]] friend constexpr bool
  operator==(const RenderPresentationSettingsDelta &,
...
inline constexpr std::uint32_t renderPresentationSettingsRecordSchemaVersion =
    4U;
...
struct RenderPresentationSettingsRecord final {
  std::uint32_t schemaVersion{renderPresentationSettingsRecordSchemaVersion};
  float renderScalePercent{native_render_policy::defaultRenderScalePercent};
  ...
  float verticalFovAdjustmentDegrees{
      native_render_policy::defaultVerticalFovAdjustmentDegrees};
  float uiScalePercent{native_render_policy::defaultUiScalePercent};
  std::uint8_t textureMode{};

  [[nodiscard]] friend constexpr bool
  operator==(const RenderPresentationSettingsRecord &,
