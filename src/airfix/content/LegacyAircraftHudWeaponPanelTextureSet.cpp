...
constexpr std::string_view catalogPrefix = "weapon_";
constexpr std::string_view catalogSuffix = ".gti";
constexpr std::string_view catalogLogicalPrefix =
    "Graphics\\Ingame\\HUD\\weapon_";
constexpr std::array<std::string_view,
                     legacyAircraftHudWeaponPanelFixedTextureCount>
    fixedLogicalPaths{{
...
void addIssue(
    LegacyAircraftHudWeaponPanelTextureLoadResult &result,
    const LegacyAircraftHudWeaponPanelTextureLoadIssueKind kind,
    const std::optional<LegacyAircraftHudWeaponPanelTextureKind> textureKind =
        std::nullopt,
...
[[nodiscard]] bool
report(LegacyAircraftHudWeaponPanelTextureLoadResult &result,
       const VerifiedContentSession &session,
       const VerifiedContentTransactionIdentity &identity,
       const ContentRevision &revision, const std::stop_token stopToken,
       const LegacyAircraftHudWeaponPanelTextureLoadProgressCallback &callback,
       const LegacyAircraftHudWeaponPanelTextureLoadPhase phase,
  ...
  if (callback) {
    try {
      ...
      requireExpectedSession(session, identity, revision);
      addIssue(result, LegacyAircraftHudWeaponPanelTextureLoadIssueKind::
...
[[nodiscard]] bool validLimits(
    const LegacyAircraftHudWeaponPanelTextureLoadLimits &limits) noexcept {
  ...
  return limits.maximumIconCount != 0U && limits.maximumIconCount <= 4096U &&
         ...
         upload.maximumDimension >=
             legacyAircraftHudWeaponPanelBackgroundWidth &&
...
[[nodiscard]] std::pair<std::uint32_t, std::uint32_t> expectedDimensions(
    const LegacyAircraftHudWeaponPanelTextureKind kind) noexcept {
  return kind == LegacyAircraftHudWeaponPanelTextureKind::icon
             ? std::pair{legacyAircraftHudWeaponPanelIconWidth,
                         legacyAircraftHudWeaponPanelIconHeight}
             : std::pair{legacyAircraftHudWeaponPanelBackgroundWidth,
                         legacyAircraftHudWeaponPanelBackgroundHeight};
}

[[nodiscard]] bool
validTexture(const LoadedLegacyAircraftHudWeaponPanelTexture &texture,
  ...
  if (texture.textureId.value != denseIndex ||
      ...
      texture.upload.format != legacyAircraftHudWeaponPanelTextureFormat ||
...
[[nodiscard]] bool
appendSource(LegacyAircraftHudWeaponPanelTextureLoadResult &result,
             std::vector<TextureSource> &sources,
             const LegacyAircraftHudWeaponPanelTextureLoadLimits &limits,
             const LegacyAircraftHudWeaponPanelTextureKind kind,
             const LegacyAircraftHudWeaponPanelTextureArchive archive,
  ...
  for (const auto &existing : sources) {
    if (existing.archive == archive && existing.fileIndex == fileIndex) {
      addIssue(
          result,
          LegacyAircraftHudWeaponPanelTextureLoadIssueKind::duplicateSource,
          kind, fileIndex);
      return false;
    }
  }
  ...
  if (entry.unpackedSize > limits.maximumSourceBytes ||
    ...
    addIssue(
        result,
        LegacyAircraftHudWeaponPanelTextureLoadIssueKind::sourceLimitExceeded,
        kind, fileIndex);
    return false;
  }
  ...
  if (!checkedAdd(totalSourceFootprint, footprint)) {
    addIssue(result,
             LegacyAircraftHudWeaponPanelTextureLoadIssueKind::integerOverflow,
             kind, fileIndex);
    return false;
  }
...
bool LoadedLegacyAircraftHudWeaponPanelTextureSet::valid() const noexcept {
  if (!transactionIdentity.valid() || iconCount == 0U ||
      textures.size() !=
          legacyAircraftHudWeaponPanelFixedTextureCount + iconCount ||
  ...
  for (std::size_t index = 0U; index < textures.size(); ++index) {
    ...
    if (index < legacyAircraftHudWeaponPanelFixedTextureCount) {
      ...
      const auto expectedIconIndex = static_cast<std::uint32_t>(
          index - legacyAircraftHudWeaponPanelFixedTextureCount);
      if (texture.kind != LegacyAircraftHudWeaponPanelTextureKind::icon ||
          texture.archive !=
          ...
          !asciiStartsWithInsensitive(texture.logicalPath,
                                      catalogLogicalPrefix) ||
          !asciiEndsWithInsensitive(texture.logicalPath, catalogSuffix) ||
          texture.logicalPath.size() <=
              catalogLogicalPrefix.size() + catalogSuffix.size() ||
          !asciiEqualInsensitive(std::string_view(texture.logicalPath)
                                     .substr(catalogLogicalPrefix.size(),
                                             texture.logicalPath.size() -
                                                 catalogLogicalPrefix.size() -
                                                 catalogSuffix.size()),
                                 texture.matchKey)) {
        return false;
      }
      for (std::size_t previous = legacyAircraftHudWeaponPanelFixedTextureCount;
...
bool LoadedLegacyAircraftHudWeaponPanelTextureSet::belongsTo(
    const VerifiedContentSession &session) const noexcept {
  return valid() && transactionIdentity == session.transactionIdentity() &&
         revision == session.revision();
}

const LoadedLegacyAircraftHudWeaponPanelTexture *
LoadedLegacyAircraftHudWeaponPanelTextureSet::background(
    const LegacyAircraftHudWeaponPanelTextureKind kind) const noexcept {
  if (!valid()) {
    return nullptr;
  }
  if (kind == LegacyAircraftHudWeaponPanelTextureKind::primaryBackground) {
    return &textures[0U];
  }
  if (kind == LegacyAircraftHudWeaponPanelTextureKind::secondaryBackground) {
    return &textures[1U];
  }
  return nullptr;
}
...
const LoadedLegacyAircraftHudWeaponPanelTexture *
LoadedLegacyAircraftHudWeaponPanelTextureSet::icon(
    const std::uint32_t iconIndex) const noexcept {
  const auto offset = legacyAircraftHudWeaponPanelFixedTextureCount +
...
const LoadedLegacyAircraftHudWeaponPanelTexture *
LoadedLegacyAircraftHudWeaponPanelTextureSet::findIconForWeaponType(
    const std::string_view weaponTypeName) const noexcept {
  if (weaponTypeName.size() <= 2U || !valid()) {
    return nullptr;
  }
  const auto key = weaponTypeName.substr(2U);
  for (std::size_t index = legacyAircraftHudWeaponPanelFixedTextureCount;
       index < textures.size(); ++index) {
    if (asciiEqualInsensitive(textures[index].matchKey, key)) {
...
LegacyAircraftHudWeaponPanelTextureLoadResult
loadLegacyAircraftHudWeaponPanelTextures(
    VerifiedContentSession &session,
    const LegacyAircraftHudWeaponPanelTextureLoadLimits &externalLimits,
  ...
  try {
    ...
    if (!report(result, session, identity, revision, stopToken, progress,
                LegacyAircraftHudWeaponPanelTextureLoadPhase::validatingInput,
                0U, legacyAircraftHudWeaponPanelFixedTextureCount)) {
      return result;
    }

    std::vector<TextureSource> sources;
    sources.reserve(legacyAircraftHudWeaponPanelFixedTextureCount +
    ...
    for (std::size_t offset = 0U; offset < fileCount; ++offset) {
      ...
      if (name.size() <= catalogPrefix.size() + catalogSuffix.size()) {
        addIssue(result,
                 LegacyAircraftHudWeaponPanelTextureLoadIssueKind::
                     catalogEntryInvalid,
                 LegacyAircraftHudWeaponPanelTextureKind::icon, fileIndex);
        return result;
      }
      const auto key =
          name.substr(catalogPrefix.size(), name.size() - catalogPrefix.size() -
                                                catalogSuffix.size());
      ...
      if (sources.size() - legacyAircraftHudWeaponPanelFixedTextureCount >=
          limits.maximumIconCount) {
        addIssue(result,
                 LegacyAircraftHudWeaponPanelTextureLoadIssueKind::
      ...
      if (!appendSource(result, sources, limits,
                        LegacyAircraftHudWeaponPanelTextureKind::icon,
                        LegacyAircraftHudWeaponPanelTextureArchive::source,
    ...
    if (sources.size() == legacyAircraftHudWeaponPanelFixedTextureCount) {
      addIssue(
          result,
          LegacyAircraftHudWeaponPanelTextureLoadIssueKind::catalogEntryInvalid,
          LegacyAircraftHudWeaponPanelTextureKind::icon);
      return result;
    }
    ...
    if (!report(
            ...
            LegacyAircraftHudWeaponPanelTextureLoadPhase::discoveringCatalog,
            totalItems, totalItems)) {
      return result;
    }

    LoadedLegacyAircraftHudWeaponPanelTextureSet candidate{
        .revision = revision,
        .transactionIdentity = identity,
        .textures = {},
        .iconCount = totalItems - legacyAircraftHudWeaponPanelFixedTextureCount,
    ...
    for (std::size_t index = 0U; index < sources.size(); ++index) {
      ...
      try {
        bytes =
            source.archive == LegacyAircraftHudWeaponPanelTextureArchive::source
        ...
        addIssue(
            result,
            LegacyAircraftHudWeaponPanelTextureLoadIssueKind::sourceReadFailure,
            source.kind, source.fileIndex);
        return result;
      }
      if (!report(result, session, identity, revision, stopToken, progress,
                  LegacyAircraftHudWeaponPanelTextureLoadPhase::readingTextures,
                  index + 1U, totalItems)) {
        return result;
      }
      ...
      if (uploadPlan.format != legacyAircraftHudWeaponPanelTextureFormat) {
        addIssue(
            result,
            LegacyAircraftHudWeaponPanelTextureLoadIssueKind::unexpectedFormat,
            source.kind, source.fileIndex);
        return result;
      }
      ...
      candidate.textures.push_back({
          ...
          .iconIndex =
              source.kind == LegacyAircraftHudWeaponPanelTextureKind::icon
      ...
      if (!report(
              result, session, identity, revision, stopToken, progress,
              LegacyAircraftHudWeaponPanelTextureLoadPhase::preparingTextures,
              index + 1U, totalItems)) {
        return result;
      }
    }
    ...
    if (!report(result, session, identity, revision, stopToken, progress,
                LegacyAircraftHudWeaponPanelTextureLoadPhase::complete,
                totalItems, totalItems)) {
      return result;
    }
    requireExpectedSession(session, identity, revision);
    ...
    addIssue(
        result,
        LegacyAircraftHudWeaponPanelTextureLoadIssueKind::allocationFailure);
    return result;
  } catch (...) {
    result.textures.reset();
    result.issues.clear();
