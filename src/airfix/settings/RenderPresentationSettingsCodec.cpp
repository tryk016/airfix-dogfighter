...
RenderSettingsCodecError::RenderSettingsCodecError(
    const RenderSettingsCodecErrorKind kind,
...
std::vector<std::uint8_t> encodeRenderSettingsDocument(
    const render::RenderPresentationSettingsRecord &record) {
  if (record.schemaVersion !=
      render::renderPresentationSettingsRecordSchemaVersion) {
    fail(RenderSettingsCodecErrorKind::invalidSchemaVersion,
         record.schemaVersion,
         "AFRS encoder accepts only the current semantic schema");
  }
  if (semanticIssue(record).has_value()) {
    fail(RenderSettingsCodecErrorKind::invalidSemanticRecord,
         record.schemaVersion, "AFRS semantic record is invalid");
  }
  ...
  appendU32(output,
            std::bit_cast<std::uint32_t>(record.verticalFovAdjustmentDegrees));
  appendFieldHeader(output, uiScaleField, 4U);
  appendU32(output, std::bit_cast<std::uint32_t>(record.uiScalePercent));
  appendFieldHeader(output, textureModeField, 1U);
  output.push_back(record.textureMode);
...
decodeRenderSettingsDocument(const std::span<const std::uint8_t> bytes,
  ...
  if (!std::equal(magic.begin(), magic.end(), bytes.begin())) {
    fail(RenderSettingsCodecErrorKind::badMagic, std::nullopt,
         "AFRS magic is invalid");
  }
  ...
  if (schemaVersion != legacySchemaVersion &&
      ...
      schemaVersion != uiScaleSchemaVersion &&
      schemaVersion != render::renderPresentationSettingsRecordSchemaVersion) {
    fail(RenderSettingsCodecErrorKind::invalidSchemaVersion, schemaVersion,
  ...
  const std::uint16_t maximumField =
      schemaVersion == legacySchemaVersion    ? diagnosticsField
      : schemaVersion == safeFovSchemaVersion ? verticalFovAdjustmentField
      : schemaVersion == uiScaleSchemaVersion ? uiScaleField
                                              : textureModeField;
  std::array<bool, textureModeField + 1U> seen{};
  ...
  while (offset < bytes.size()) {
    ...
    const auto expectedSize = field == renderScaleField ||
    ...
    case renderScaleField:
      ...
      break;
    case scenePresentationField:
      record.scenePresentation = bytes[offset];
      break;
    case visualProfileField:
      record.visualProfile = bytes[offset];
      break;
    case diagnosticsField:
      record.diagnosticsOverlayEnabled = bytes[offset];
      break;
    case verticalFovAdjustmentField:
      ...
      break;
    case uiScaleField:
      record.uiScalePercent = std::bit_cast<float>(readU32(bytes, offset));
      break;
    case textureModeField:
      record.textureMode = bytes[offset];
      break;
    default:
      throw std::logic_error("validated AFRS field is unreachable");
    }
    offset += size;
  }
  for (std::uint16_t field = renderScaleField; field <= maximumField; ++field) {
    if (!seen[field]) {
