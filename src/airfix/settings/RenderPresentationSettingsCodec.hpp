...
public:
  ...
  [[nodiscard]] RenderSettingsCodecErrorKind kind() const noexcept {
    return kind_;
  }
...
struct OpaqueFutureRenderSettingsRecord final {
  ...
  operator==(const OpaqueFutureRenderSettingsRecord &,
             const OpaqueFutureRenderSettingsRecord &) = default;
};

using DecodedRenderSettingsDocument =
    std::variant<render::RenderPresentationSettingsRecord,
                 OpaqueFutureRenderSettingsRecord>;
...
// order. Schema 1 migrates with safe FOV, UI scale, and Classic texture mode at
// their exact defaults; schema 2 migrates with default UI scale and Classic
// texture mode; schema 3 migrates with Classic texture mode. A structurally
// intact future schema is returned as opaque exact bytes so a downgrade can
// preserve it without interpreting or rewriting it.
[[nodiscard]] DecodedRenderSettingsDocument decodeRenderSettingsDocument(
    std::span<const std::uint8_t> bytes,
    std::size_t maximumBytes = maximumRenderSettingsDocumentBytes);

[[nodiscard]] std::vector<std::uint8_t> encodeRenderSettingsDocument(
