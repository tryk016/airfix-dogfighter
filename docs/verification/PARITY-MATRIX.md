...
| Aircraft flight model | P0 | observed | SCN-FLIGHT-001 | Scheduler, controls, complete static equations, and the signed 64-bit two-second rest/sleep gate are recovered; the sleep transition is implemented as an isolated contract while physical units, runtime traces, and numeric tolerance remain unknown |
