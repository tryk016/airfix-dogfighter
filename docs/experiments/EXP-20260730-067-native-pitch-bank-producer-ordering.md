...
|---|---|---|---|
| `Dogfighter.exe!InputAndTimePump` | `[0x00410FA0,0x00410FE3)` | `[0x00010FA0,0x00010FE3)` | explicit vtable entry |
...
| `+0x14` | `InputAndTimePump` | called at `0x004161D0` |
| `+0x18` | render-event producer | called at `0x004161DB` |

`InputAndTimePump` calls `DispatchInputEvents` at `0x00410FA7`, performs
...
|---|---|---|
| local device priority and joystick BANK-before-PITCH | high | DirectInput driver's internal buffer order is outside the image |
| keyboard callback and binding execution order | high | physical key-repeat cadence is OS/driver/runtime behavior |
| AI PITCH-before-BANK and change suppression | high | `_ftol` halfway result depends on live x87 control |
| producer-specific payload bounds | high, analog conditional on installed range being honored | no downstream assumption replaces driver compliance |
| local-input-before-SetTime and render-after-time | high | wall-clock iteration frequency is not encoded |
| zero/one/many 12 ms refreshes per SetTime | high | full target-mask/network policy is not exhaustively classified |
| effective nominal AI trigger after nine 12 ms steps | high for uninterrupted `dt=0.012f` | catch-up and skipped publications can compress or delay wall time |
...
Static evidence is sufficient for immediate local call order but not for
...
- The new targeted Ghidra reference exporter compiled and ran headlessly.
  Forty-three selected local Ghidra reports have no unresolved-address marker.
...
- The function catalog has 276 rows, 14 columns, unique stable IDs, and unique
  module/RVA pairs; only its two pre-existing missing CC references remain.
- Synthetic public-boundary tests and the 548-file repository scan pass.
