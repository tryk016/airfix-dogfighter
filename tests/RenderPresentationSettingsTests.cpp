...
static_assert(std::is_trivially_copyable_v<RenderPresentationSettings>);
static_assert(std::is_trivially_copyable_v<RenderPresentationSettingsRecord>);
...
void requireIssue(const std::optional<RenderPresentationSettingsIssue> &issue,
                  const RenderPresentationSettingsIssueKind expected,
                  const char *const message) {
  require(issue.has_value() && issue->kind == expected, message);
}
...
void testDefaultsAndBoundaries() {
  ...
  require(
      defaults ==
              RenderPresentationSettings{
                  ...
                  .scenePresentation = ScenePresentationMode::widescreenHorPlus,
  ...
  for (const float boundary : std::array{50.0F, 200.0F}) {
    const auto result = resolveRenderPresentationSettings(
        defaults, {.renderScalePercent = boundary});
    require(result.accepted() && result.settings.renderScalePercent == boundary,
            "valid render-scale boundary was rejected");
  }
  for (const float boundary : std::array{0.0F, 25.0F}) {
    const auto result = resolveRenderPresentationSettings(
        defaults, {.verticalFovAdjustmentDegrees = boundary});
    require(result.accepted() &&
                result.settings.verticalFovAdjustmentDegrees == boundary,
            "valid vertical-FOV boundary was rejected");
  }
  for (const float boundary : std::array{75.0F, 150.0F}) {
    const auto result = resolveRenderPresentationSettings(
        defaults, {.uiScalePercent = boundary});
    require(result.accepted() && result.settings.uiScalePercent == boundary,
...
void testInvalidSettingsAndAtomicResolve() {
  const RenderPresentationSettings base{
      ...
      .scenePresentation = ScenePresentationMode::originalFourByThree,
  ...
  for (const auto &test : scaleCases) {
    const auto result = resolveRenderPresentationSettings(
        base, {.renderScalePercent = test.value});
    require(!result.accepted(), "invalid render scale was accepted");
    require(result.settings == base,
  ...
  for (const auto &test : fovCases) {
    const auto result = resolveRenderPresentationSettings(
        base, {.verticalFovAdjustmentDegrees = test.value});
  ...
  for (const auto &test : std::array{
    ...
    const auto result =
        resolveRenderPresentationSettings(base, {.uiScalePercent = test.value});
    require(!result.accepted() && result.settings == base,
  ...
  const auto forgedPresentation = static_cast<ScenePresentationMode>(0xFFU);
  const auto presentationResult = resolveRenderPresentationSettings(
      base, {.scenePresentation = forgedPresentation});
  require(!presentationResult.accepted() && presentationResult.settings == base,
  ...
  const auto profileResult =
      resolveRenderPresentationSettings(base, {.visualProfile = forgedProfile});
  require(!profileResult.accepted() && profileResult.settings == base,
          "forged visual-profile enum changed settings");
  requireIssue(profileResult.issue,
               RenderPresentationSettingsIssueKind::unsupportedVisualProfile,
               "forged visual-profile enum reported the wrong issue");

  const auto forgedTextureMode =
      static_cast<airfix::texture::TextureMode>(0xFFU);
  const auto textureModeResult = resolveRenderPresentationSettings(
      base, {.textureMode = forgedTextureMode});
  require(!textureModeResult.accepted() && textureModeResult.settings == base,
          "forged texture-mode enum changed settings");
  requireIssue(textureModeResult.issue,
               RenderPresentationSettingsIssueKind::unsupportedTextureMode,
               "forged texture-mode enum reported the wrong issue");
  ...
  require(!recordResult.complete() && !recordResult.record.has_value(),
          "invalid settings produced a semantic record");
  requireIssue(recordResult.issue,
               RenderPresentationSettingsIssueKind::unsupportedVisualProfile,
...
void testInvalidBaseIsRejectedBeforeOverrides() {
  ...
  base.renderScalePercent = std::numeric_limits<float>::quiet_NaN();
  ...
  require(!result.accepted() &&
              ...
              result.settings.scenePresentation == base.scenePresentation &&
  ...
  base = {};
  base.scenePresentation = static_cast<ScenePresentationMode>(0xFFU);
  ...
  result = resolveRenderPresentationSettings(
      base, {.visualProfile = VisualProfile::classic});
  require(!result.accepted() &&
              result.settings.visualProfile == base.visualProfile,
  ...
  base = {};
  base.textureMode = static_cast<airfix::texture::TextureMode>(0xFFU);
  result = resolveRenderPresentationSettings(
      base, {.textureMode = airfix::texture::TextureMode::classic});
  require(!result.accepted() && result.settings.textureMode == base.textureMode,
          "override repaired a forged base texture mode");
  requireIssue(result.issue,
               RenderPresentationSettingsIssueKind::unsupportedTextureMode,
               "forged base texture mode reported the wrong issue");
}
...
void testPartialOverridesPreserveOtherValues() {
  ...
  const auto scaleOnly =
      resolveRenderPresentationSettings(base, {.renderScalePercent = 150.0F});
  require(scaleOnly.accepted() &&
              scaleOnly.settings ==
                  RenderPresentationSettings{
  ...
  const auto diagnosticsOnly = resolveRenderPresentationSettings(
      base, {.diagnosticsOverlayEnabled = false});
  require(diagnosticsOnly.accepted() &&
              ...
              diagnosticsOnly.settings.textureMode ==
                  airfix::texture::TextureMode::enhanced &&
              !diagnosticsOnly.settings.diagnosticsOverlayEnabled &&
              diagnosticsOnly.settings.verticalFovAdjustmentDegrees == 15.0F,
          "partial diagnostics override replaced unrelated settings");

  const auto fovOnly = resolveRenderPresentationSettings(
      base, {.verticalFovAdjustmentDegrees = 20.0F});
  require(fovOnly.accepted() && fovOnly.settings.renderScalePercent == 75.0F &&
              fovOnly.settings.scenePresentation ==
                  ScenePresentationMode::originalFourByThree &&
              fovOnly.settings.visualProfile == VisualProfile::enhanced &&
              fovOnly.settings.textureMode ==
                  airfix::texture::TextureMode::enhanced &&
  ...
  const auto empty = resolveRenderPresentationSettings(base, {});
  require(empty.accepted() && empty.settings == base,
          "empty override changed settings");
}

void testSettingsDelta() {
  const RenderPresentationSettings base;
  const auto noChange = diffRenderPresentationSettings(base, base);
  require(noChange.complete() && noChange.delta.has_value() &&
  ...
  candidate = base;
  candidate.scenePresentation = ScenePresentationMode::originalFourByThree;
  ...
  candidate = base;
  candidate.textureMode = airfix::texture::TextureMode::enhanced;
  result = diffRenderPresentationSettings(base, candidate);
  require(result.complete() && result.delta.has_value() &&
              *result.delta ==
                  RenderPresentationSettingsDelta{
          ...
          "texture-mode delta changed renderer presentation domains");
  ...
  const auto combined = diffRenderPresentationSettings(base, candidate);
  require(combined.complete() && combined.delta.has_value() &&
              *combined.delta ==
                  RenderPresentationSettingsDelta{
                      .scaleTargetsChanged = true,
                      .layoutChanged = true,
...
void testInvalidSettingsProduceNoDelta() {
  ...
  auto invalid = valid;
  invalid.renderScalePercent = std::numeric_limits<float>::quiet_NaN();
  ...
  result = diffRenderPresentationSettings(valid, invalid);
  require(!result.complete() && !result.delta.has_value(),
          "forged candidate texture mode produced a delta");
  requireIssue(result.issue,
               RenderPresentationSettingsIssueKind::unsupportedTextureMode,
               "forged candidate texture mode reported the wrong issue");
}

[[nodiscard]] NativeRenderLayoutConfig
layoutConfig(const RenderPresentationSettings &settings) {
  return {
...
void testDiagnosticsAndProfileAreLayoutOrthogonal() {
  ...
  require(baseConfig.renderScalePercent == changedConfig.renderScalePercent &&
              baseConfig.scenePresentation == changedConfig.scenePresentation &&
  ...
  const auto changedLayout = buildNativeRenderLayout(changedConfig);
  require(baseLayout.complete() && changedLayout.complete(),
          "orthogonal settings produced an invalid layout");
  require(baseLayout.layout->outputExtent() ==
              ...
              baseLayout.layout->sceneViewportInRenderTarget() ==
                  changedLayout.layout->sceneViewportInRenderTarget() &&
...
void testFovSettingChangesOnlyProjectionLayout() {
  ...
  const auto baseLayout = buildNativeRenderLayout(layoutConfig(base));
  const auto changedLayout = buildNativeRenderLayout(layoutConfig(changed));
  require(baseLayout.complete() && changedLayout.complete(),
  ...
  require(changedLayout.layout->verticalFovAdjustmentDegrees() == 20.0F &&
              changedLayout.layout->verticalFovDegrees() >
                  baseLayout.layout->verticalFovDegrees() &&
              changedLayout.layout->horizontalFovDegrees() >
                  baseLayout.layout->horizontalFovDegrees() &&
...
void testStorageNeutralRoundTrip() {
  ...
  const auto recordResult = makeRenderPresentationSettingsRecord(settings);
  require(recordResult.complete() && recordResult.record.has_value() &&
              ...
              recordResult.record->textureMode == 1U &&
              recordResult.record->diagnosticsOverlayEnabled == 1U &&
              recordResult.record->verticalFovAdjustmentDegrees == 12.0F &&
              recordResult.record->uiScalePercent == 125.0F,
          "settings did not produce the versioned semantic record");

  const auto restored =
      renderPresentationSettingsFromRecord(*recordResult.record);
  require(restored.complete() && restored.settings == settings,
...
void testInvalidStorageRecordsFailClosed() {
  ...
  for (const std::uint32_t schema :
       std::array{0U, renderPresentationSettingsRecordSchemaVersion + 1U}) {
    auto record = baseline;
    record.schemaVersion = schema;
    const auto restored = renderPresentationSettingsFromRecord(record);
    require(!restored.complete() && !restored.settings.has_value(),
            "unknown settings schema produced settings");
    requireIssue(restored.issue,
  ...
  auto restored = renderPresentationSettingsFromRecord(record);
  require(!restored.complete() && !restored.settings.has_value(),
  ...
  record = baseline;
  record.textureMode = 2U;
  restored = renderPresentationSettingsFromRecord(record);
  require(!restored.complete() && !restored.settings.has_value(),
          "invalid stored texture mode produced settings");
  requireIssue(restored.issue,
               RenderPresentationSettingsIssueKind::unsupportedTextureMode,
               "invalid stored texture mode reported the wrong issue");
...
int main() {
  try {
    ...
    std::cerr << "Render presentation settings tests failed: " << error.what()
              << '\n';
    return 1;
  }
}
