...
[[nodiscard]] ControllerInputBatch batch(
...
void fillSentinel(OutputBuffer& output) {
    output.fill(sentinel);
}

void requireUntouched(
    const OutputBuffer& output, const std::string& context) {
    for (const auto& emission : output) {
        require(emission == sentinel, context + " changed caller output");
    }
}

void requireEmission(
    const ControllerInputEmission& actual,
...
void establishNeutral(
    ControllerInputBatchBridge& bridge,
    const std::uint64_t generation = 1U) {
    OutputBuffer output{};
    require(
        accept(bridge, batch(generation, {}, {}), output) ==
...
void testAllDigitalMappings() {
    ...
    require(
        !controllerDigitalMapping(ControllerDigitalControl::count)
             .control.valid(),
...
void testNeutralFullState() {
    ...
    for (std::size_t index = 0U; index < axisControls.size(); ++index) {
        requireEmission(output[index], axisControls[index],
            PhysicalEventKind::analog, 0, "neutral full-state axis");
    }
    for (std::size_t index = 0U;
        ...
        requireEmission(
            output[controllerAxisCount + index],
            mapping.control,
            mapping.kind,
            0,
...
void testNonNeutralFullStateAndDeadzone() {
    ...
    for (std::size_t index = 0U;
         ...
         ++index) {
        require(
            output[controllerAxisCount + index].value == q15One,
...
void testMeaningfulAxisChanges() {
    ...
    require(
        accept(
            bridge,
            batch(1U, initial, belowThreshold),
            output) == 0U,
    ...
    require(
        accept(
            bridge,
            batch(1U, belowThreshold, atThreshold),
            output) == 3U,
        "threshold and deadzone exit should emit exactly three axes");
    requireEmission(output[0U],
        airfix::input::controls::controller::leftStickX,
        PhysicalEventKind::analog, 6024, "bank threshold");
    requireEmission(output[1U],
        airfix::input::controls::controller::leftStickY,
        PhysicalEventKind::analog, -6024, "pitch threshold");
    requireEmission(output[2U],
        airfix::input::controls::controller::rightStickX,
        PhysicalEventKind::analog, 4096, "look deadzone exit");
    ...
    require(
        accept(
            bridge,
            batch(1U, atThreshold, reenterDeadzone),
            output) == 1U,
...
void testCompleteTapsForEveryDigitalControl() {
    ...
    for (std::size_t index = 0U;
         ...
         ++index) {
        const auto mapping = controllerDigitalMapping(digitalControl(index));
        requireEmission(output[index * 2U], mapping.control, mapping.kind,
            q15One, "tap press");
        requireEmission(output[index * 2U + 1U], mapping.control, mapping.kind,
            0, "tap release");
    }
}
...
void testMixedOrderedEdgesAndFinalReconciliation() {
    ...
    alreadyReconciled.edges[0U] = {
        3U, 5U, ControllerDigitalControl::secondaryTrigger, false};
...
void requireRejectedAtomically(
    ...
    require(
        bridge.currentGeneration() == generation,
...
void testMalformedBatchesAreAtomic() {
    ...
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidGeneration, "zero generation");

    malformed = valid;
    malformed.generation = 8U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidGeneration, "generation rollback");

    malformed = valid;
    malformed.overflowed = true;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::inputOverflow, "producer overflow");

    malformed = valid;
    malformed.edgeCount = ControllerInputBatch::edgeCapacity + 1U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::inputCapacityExceeded, "edge capacity");

    malformed = valid;
    malformed.startingState.pausePressed = true;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::startingStateMismatch, "starting state");

    malformed = valid;
    malformed.startingState.bank = static_cast<std::int16_t>(-32768);
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidAxis, "starting Q15");

    malformed = valid;
    malformed.finalState.lookY = static_cast<std::int16_t>(-32768);
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidAxis, "final Q15");

    malformed = valid;
    malformed.edges[0U] = {
        8U, 1U, ControllerDigitalControl::pause, true};
    malformed.edgeCount = 1U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidEdgeGeneration, "edge generation");

    malformed = valid;
    malformed.edges[0U] = {
        9U, 0U, ControllerDigitalControl::pause, true};
    malformed.edgeCount = 1U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidEdgeOrder, "zero edge order");

    malformed = valid;
    malformed.edges[0U] = {
        9U, 2U, ControllerDigitalControl::pause, true};
    malformed.edges[1U] = {
        9U, 2U, ControllerDigitalControl::pause, false};
    malformed.edgeCount = 2U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidEdgeOrder, "non-increasing edge order");
    ...
    malformed.edgeCount = 1U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidEdgeControl, "edge control range");

    malformed = valid;
    malformed.edges[0U] = {
        9U, 1U, ControllerDigitalControl::pause, false};
    malformed.edgeCount = 1U;
    requireRejectedAtomically(bridge, malformed,
        ControllerBatchStatus::invalidEdgeTransition, "duplicate edge state");
    ...
    oneAcceptedEdge.edges[0U] = {
        9U, 10U, ControllerDigitalControl::pause, true};
    ...
    staleAcrossBatches.edges[0U] = {
        9U, 10U, ControllerDigitalControl::pause, false};
    staleAcrossBatches.edgeCount = 1U;
    staleAcrossBatches.finalState.pausePressed = false;
    requireRejectedAtomically(bridge, staleAcrossBatches,
        ControllerBatchStatus::invalidEdgeOrder, "cross-batch edge order");
}
...
void testOutputCapacityAndMaximumBound() {
    ...
    require(
        tooSmall.status == ControllerBatchStatus::outputCapacityExceeded,
    ...
    std::uint64_t order = 1U;
    for (std::size_t index = 0U;
         index < ControllerInputBatch::edgeCapacity;
         ++index) {
        const auto control =
    ...
    for (std::size_t index = 0U;
         ...
         ++index) {
        const auto control = digitalControl(index);
        maximum.finalState.setPressed(
            control, !maximum.finalState.pressed(control));
    }

    ControllerInputBatchBridge maximumBridge;
    fillSentinel(output);
    const auto bounded = maximumBridge.process(
        maximum,
        std::span{output}.first(
            ControllerInputBatchBridge::maximumEmissionCount - 1U));
    require(
        bounded.status == ControllerBatchStatus::outputCapacityExceeded,
...
void testDeterministicReplay() {
    ...
    first.edges[0U] = {
        41U, 3U, ControllerDigitalControl::uiConfirm, true};
    first.edges[1U] = {
        41U, 4U, ControllerDigitalControl::uiConfirm, false};
    first.edgeCount = 2U;
    ...
    for (std::size_t index = 0U;
         index < leftFirst.emissionCount;
         ++index) {
        require(
            leftOutput[index] == rightOutput[index],
    ...
    for (std::size_t index = 0U;
         index < leftSecond.emissionCount;
         ++index) {
        require(
            leftOutput[index] == rightOutput[index],
...
static_assert(noexcept(
    std::declval<ControllerInputBatchBridge&>().process(
        std::declval<const ControllerInputBatch&>(),
        std::declval<std::span<ControllerInputEmission>>())));
static_assert(noexcept(
    std::declval<ControllerInputBatchBridge&>().reset()));
...
int main() {
    try {
        ...
        std::cerr << "ControllerInputBatchBridgeTests failed: "
                  << error.what() << '\n';
        return 1;
    }
