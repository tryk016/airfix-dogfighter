...
class TempDirectory final {
  public:
    TempDirectory() {
        ...
        for (unsigned attempt = 0U; attempt < 100U; ++attempt) {
            root_ =
                ...
                ("airfix-settings-store-" + std::to_string(stamp) + "-" + std::to_string(attempt));
    ...
    [[nodiscard]] const std::filesystem::path& settings() const { return settings_; }

    [[nodiscard]] std::filesystem::path current() const { return settings_ / currentName; }

    [[nodiscard]] std::filesystem::path backup() const { return settings_ / backupName; }
...
[[nodiscard]] airfix::render::RenderPresentationSettings settings(const float scale,
                                                                  const bool diagnostics = false) {
    return {
        .renderScalePercent = scale,
        .scenePresentation = airfix::render::ScenePresentationMode::widescreenHorPlus,
...
[[nodiscard]] std::vector<std::uint8_t> readFile(const std::filesystem::path& path) {
    return airfix::io::readBoundedRegularFile(path,
                                              airfix::settings::maximumRenderSettingsDocumentBytes);
}

void writeRaw(const std::filesystem::path& path, const std::span<const std::uint8_t> bytes) {
...
void requireStoreError(const std::function<void()>& action,
    ...
    try {
        ...
        require(std::string(error.what()).find(privatePath.string()) == std::string::npos,
...
void testMissingSaveAndRotation() {
    ...
    const auto missing = airfix::settings::loadRenderPresentationSettings(temporary.settings());
    require(missing.source == airfix::settings::RenderSettingsLoadSource::defaults &&
    ...
    const auto firstSave =
        airfix::settings::saveRenderPresentationSettings(temporary.settings(), first);
    require(firstSave.status == airfix::settings::RenderSettingsSaveStatus::committed &&
                !firstSave.backupRotated,
            "first settings save had the wrong result");
    require(airfix::settings::loadRenderPresentationSettings(temporary.settings()).settings ==
                first,
            "saved settings did not survive a restart");

    const auto unchanged =
        airfix::settings::saveRenderPresentationSettings(temporary.settings(), first);
    require(unchanged.status == airfix::settings::RenderSettingsSaveStatus::unchanged &&
                !unchanged.backupRotated,
            "idempotent save rewrote settings");

    const auto second = settings(150.0F, true);
    const auto secondSave =
        airfix::settings::saveRenderPresentationSettings(temporary.settings(), second);
    require(secondSave.backupRotated, "changed settings did not rotate the valid current");
    require(airfix::settings::loadRenderPresentationSettings(temporary.settings()).settings ==
                second,
    ...
    const auto recovered = airfix::settings::loadRenderPresentationSettings(temporary.settings());
    require(recovered.source == airfix::settings::RenderSettingsLoadSource::backup &&
...
void testMalformedAndFuturePreservation() {
    TempDirectory temporary;
    (void)airfix::settings::saveRenderPresentationSettings(temporary.settings(), settings(80.0F));
    writeRaw(temporary.current(), std::array<std::uint8_t, 4U>{'B', 'A', 'D', '!'});
    const auto replacement = settings(125.0F);
    const auto saved =
        airfix::settings::saveRenderPresentationSettings(temporary.settings(), replacement);
    require(!saved.backupRotated &&
                airfix::settings::loadRenderPresentationSettings(temporary.settings()).settings ==
                    replacement,
            "malformed current was promoted or not replaced");

    const auto futureBase = settings(130.0F);
    const auto futureBaseSaved =
        airfix::settings::saveRenderPresentationSettings(temporary.settings(), futureBase);
    ...
    const auto loaded = airfix::settings::loadRenderPresentationSettings(temporary.settings());
    require(loaded.source == airfix::settings::RenderSettingsLoadSource::defaults &&
                loaded.persistenceBlocked &&
                loaded.current.status == airfix::settings::RenderSettingsFileStatus::futureSchema,
...
void replaceHook(const std::filesystem::path& prepared, const std::filesystem::path& target,
                 void* const opaque) {
...
void throwRetry(const std::filesystem::path&, const std::filesystem::path&, void*) {
...
void testFailureAndAmbiguousReadback() {
    ...
    (void)airfix::settings::saveRenderPresentationSettings(temporary.settings(), first);
    ...
    requireStoreError(
        [&] {
            (void)airfix::settings::testing::saveRenderPresentationSettingsWithHooks(
                temporary.settings(), settings(120.0F), beforeHooks);
        },
        airfix::settings::RenderSettingsStoreErrorKind::saveFailed, temporary.settings());
    require(airfix::settings::loadRenderPresentationSettings(temporary.settings()).settings ==
                first,
    ...
    const auto confirmed = airfix::settings::testing::saveRenderPresentationSettingsWithHooks(
        temporary.settings(), settings(140.0F), afterHooks);
    require(confirmed.status == airfix::settings::RenderSettingsSaveStatus::committedAfterReadback,
    ...
    requireStoreError(
        ...
        airfix::settings::RenderSettingsStoreErrorKind::commitUnknown, temporary.settings());
}
...
void testOversizedAndLinkedRecords() {
    {
        ...
        const auto loaded = airfix::settings::loadRenderPresentationSettings(temporary.settings());
        require(loaded.current.status == airfix::settings::RenderSettingsFileStatus::oversized &&
...

    {
        ...
        (void)airfix::settings::saveRenderPresentationSettings(temporary.settings(),
                                                               settings(90.0F));
        std::error_code error;
        std::filesystem::create_hard_link(temporary.current(),
                                          temporary.settings() / "linked-alias.afrs", error);
...
void testLinkedSettingsDirectory() {
    ...
    require(std::filesystem::create_directory(target), "cannot create linked settings target");
    const auto record = airfix::render::makeRenderPresentationSettingsRecord(settings(135.0F));
    require(record.complete(), "linked settings fixture is invalid");
    const auto bytes = airfix::settings::encodeRenderSettingsDocument(*record.record);
    writeRaw(target / currentName, bytes);

    std::error_code error;
    std::filesystem::create_directory_symlink(target, temporary.settings(), error);
    if (error) {
    ...
    requireStoreError(
        ...
        airfix::settings::RenderSettingsStoreErrorKind::invalidDirectory, temporary.settings());
}
...
int main() {
    try {
        ...
        std::cerr << "Render presentation settings store tests failed: " << error.what() << '\n';
        return 1;
    }
}
